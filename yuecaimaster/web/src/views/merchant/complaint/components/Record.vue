  ...
  <div>
    <bar>
      ...
      <template v-slot:position2
        >已通过：<span class="text-blue-500">{{ statistics.count3 }}</span></template
      >
      <template v-slot:position3
        >已拒绝：<span class="text-blue-500">{{ statistics.count4 }}</span></template
      >
    </bar>
    <table-tree :tableData="form.tableData">
      ...
      >
      <el-table-column label="联系电话"
        ><template slot-scope="scope">{{ scope.row.status }}</template></el-table-column
      >
...
export default {
  ...
  methods: {
    async getListFunc() {
      ...
      let dateArr2 = Object.assign(this.searchForm).date2
      const res = await this.http.post("/web/appeal/getReviewed", {
        ...
        date3: isSearch && dateArr2 ? dateArr2[0] : "",
        date4: isSearch && dateArr2 ? dateArr2[1] : "",
      })
      if (res.code == 200) {
