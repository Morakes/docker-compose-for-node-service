  ...
  <div>
    <bar>
      <template v-slot:position1
        >总名胜数：<span class="text-blue-500">{{ statistics.count1 }}</span></template
      >
    ...
    <table-tree :tableData="form.tableData">
      <el-table-column label="特产信息" width="500px">
        <template slot-scope="scope">
          <div class="flex">
            <img :src="scope.row.cover" style="width: 200px; height: 100px" />
            <div class="ml-4">
              {{ scope.row.name }}
              <div class="mt-14">{{ "广州市从化区" + scope.row.specificAddress }}</div>
            </div>
          </div>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    async getListFunc() {
      ...
      const res = await this.http.post("/web/scenic/getScenic", {
        page: this.page.page,
    ...
    // 获取统计
    async getStatistics() {
      const RES = await this.http.get("/web/scenic/getStatistics")
      if (RES.code == 200) {
    ...
    async operFunc(oper, row) {
      switch (oper) {
        case "add":
          this.$router.push("/module/scenic/details")
          break
        case "edit":
          this.$router.push({
    ...
    async udFunc(params, row) {
      if (row.isShow == 1) {
        this.$message("该名胜正在展示中，请先移除展示")
        return
      }
      const res = await this.http.put("/web/scenic/updScenic", {
        status: params,
        id: row.id,
      })
      if (res.code == 200) {
    ...
    delFunc(row) {
      this.$confirm("是否删除", "提示", {
        ...
        .then(async () => {
          ...
          const res = await this.http.delete("/web/scenic/delScenic" + `/${row.id}`)
          if (res.code == 200) {
            this.getListFunc()
            this.$message({
              type: "success",
    ...
    async showFunc(row) {
      if (row.status == "已下架") {
        this.$message("该名胜被下架，请先上架")
        return
      }
      const res = await this.http.post("/web/scenic/setDisplay", {
        id: row.id,
      })
      if (res.code == 200) {
        this.getListFunc()
