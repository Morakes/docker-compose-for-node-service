...
export default {
  ...
  methods: {
    // get base data
    async getListFunc() {
      const res = await this.http.get("/web/specialty/getShow")
      if (res.code == 200) {
    ...
    // submit sort
    async confirmFunc() {
      const res = await this.http.post("/web/specialty/rank", {
    ...
    async operFunc(oper, row) {
      switch (oper) {
        case "edit":
          ...
          break
        case "del":
          const res = await this.http.post("/web/specialty/removeDisplay", {
            id: row.id,
