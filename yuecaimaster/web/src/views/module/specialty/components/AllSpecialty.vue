  ...
  <div>
    <bar>
      <template v-slot:position1
        >总特产数：<span class="text-blue-500">{{ statistics.count1 }}</span></template
      >
      ...
      <el-table-column label="特产信息" width="500px">
        <template slot-scope="scope">
          <div class="flex">
            ...
            <div class="ml-4">
              {{ scope.row.name }}
              <div class="mt-14">{{ scope.row.townId }}</div>
            </div>
          </div>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    async getListFunc() {
      ...
      const res = await this.http.post("/web/specialty/getSpecialty", {
        page: this.page.page,
    ...
    // 获取统计
    async getStatistics() {
      const RES = await this.http.get("/web/specialty/getStatistics")
      if (RES.code == 200) {
    ...
    async operFunc(oper, row) {
      switch (oper) {
        case "add":
          this.$router.push("/module/specialty/details")
          break
        case "edit":
          this.$router.push({
    ...
    async udFunc(params, row) {
      if (row.isShow == 1) {
        this.$message("该特产正在展示中，请先移除展示")
        return
      }
      const res = await this.http.put("/web/specialty/updSpecialty", {
        status: params,
        id: row.id,
      })
      if (res.code == 200) {
    ...
    delFunc(row) {
      this.$confirm("是否删除", "提示", {
        ...
        .then(async () => {
          ...
          const res = await this.http.delete("/web/specialty/delSpecialty" + `/${row.id}`)
          if (res.code == 200) {
            this.getListFunc()
            this.$message({
              type: "success",
    ...
    async showFunc(row) {
      if (row.status == "已下架") {
        this.$message("该特产被下架，请先上架")
        return
      }
      const res = await this.http.post("/web/specialty/setDisplay", {
        id: row.id,
      })
      if (res.code == 200) {
        this.getListFunc()
